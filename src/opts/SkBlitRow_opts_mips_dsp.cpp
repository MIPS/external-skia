...
void S32_D565_Blend_mips_dsp(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 2) {
        asm volatile (
            ...
            "shra.ph         %[t0], %[s0], 5         \n\t"
            "and             %[t2], %[t0], %[s6]     \n\t"  // t2 = 0|g1|0|g2
#if (SK_MIPS_DSP_REV >= 2)
            "shrl.ph         %[t3], %[s0], 11        \n\t"
#else
            "shra.ph         %[t0], %[s0], 11        \n\t"  // Get two reds from two Dst pixels
            "and             %[t3], %[t0], %[s5]     \n\t"
#endif // (SK_MIPS_DSP_REV >= 2)
...
void S32A_D565_Opaque_Dither_mips_dsp(uint16_t* __restrict__ dst,
...
void S32_D565_Opaque_Dither_mips_dsp(uint16_t* __restrict__ dst,
                                        const SkPMColor* __restrict__ src,
                                        int count, U8CPU alpha, int x, int y) {
...
void S32_D565_Blend_Dither_mips_dsp(uint16_t* dst,
                                       const SkPMColor* src,
                                       int count, U8CPU alpha, int x, int y) {
    register int32_t t0 = 0, t1 = 0, t2 = 0, t3 = 0, t4 = 0, t5 = 0, t6 = 0;
    register int32_t s0 = 0, s1 = 0, s2 = 0, s3 = 0;
...
void S32A_D565_Opaque_mips_dsp(uint16_t* __restrict__ dst,
                                  const SkPMColor* __restrict__ src,
                                  int count, U8CPU alpha, int x, int y) {
...
void S32A_D565_Blend_mips_dsp(uint16_t* SK_RESTRICT dst,
                                 const SkPMColor* SK_RESTRICT src, int count,
                                 U8CPU alpha, int /*x*/, int /*y*/) {
...
void S32_Blend_BlitRow32_mips_dsp(SkPMColor* SK_RESTRICT dst,
...
static const SkBlitRow::Proc platform_4444_procs_mips_dsp[] = {
    // no dither
...
SkBlitRow::Proc SkBlitRow::PlatformProcs4444(unsigned flags) {
    return platform_4444_procs_mips_dsp[flags];
}
