...
#if defined(__mips_dsp) && !defined(SK_CPU_BENDIAN)

void SI8_D16_nofilter_DX_mips(
    const SkBitmapProcState& s,
    const uint32_t* SK_RESTRICT xy,
    int count,
    uint16_t* SK_RESTRICT colors) __attribute__((optimize("O1")));

void SI8_D16_nofilter_DX_mips(const SkBitmapProcState& s,
                              const uint32_t* SK_RESTRICT xy,
    ...
    if (1 == s.fBitmap->width()) {
        ...
        asm volatile (
            ...
            "lbux    $t6, $t6(%[srcAddr])      \n\t"
            "lbux    $t7, $t7(%[srcAddr])      \n\t"
            "addu    $t0, $t0, $t0             \n\t"  // double pixels for RGB565 lookup
            "addu    $t1, $t1, $t1             \n\t"
            "addu    $t2, $t2, $t2             \n\t"
            "addu    $t3, $t3, $t3             \n\t"
            "addu    $t4, $t4, $t4             \n\t"
            ...
            "addu    $t7, $t7, $t7             \n\t"
            "pref    0, 16(%[table])           \n\t"
            "lhx     $t0, $t0(%[table])        \n\t"  // load pixels RGB565 from colmap
            "lhx     $t1, $t1(%[table])        \n\t"
            "lhx     $t2, $t2(%[table])        \n\t"
            "lhx     $t3, $t3(%[table])        \n\t"
            "lhx     $t4, $t4(%[table])        \n\t"
...
void SI8_opaque_D32_nofilter_DX_mips(
    const SkBitmapProcState& s,
    const uint32_t* SK_RESTRICT xy,
    int count,
    SkPMColor* SK_RESTRICT colors) __attribute__((optimize("O1")));

void SI8_opaque_D32_nofilter_DX_mips(const SkBitmapProcState& s,
                                     const uint32_t* SK_RESTRICT xy,
    ...
    if (1 == s.fBitmap->width()) {
        ...
        asm volatile (
            ...
            "srl     $t8, %[count], 4          \n\t"  // t8 = how many multiples of 16 src pixels
            "beqz    $t8, 3f                   \n\t"  // branch if less than 16 src pixels
            " nop                              \n\t"
            ...
            "pref    0, 64(%[table])           \n\t"
            "lwx     $t0, $t0(%[table])        \n\t"  // load pixels SkPMColor from colmap
            "lwx     $t1, $t1(%[table])        \n\t"
            "lwx     $t2, $t2(%[table])        \n\t"
            "lwx     $t3, $t3(%[table])        \n\t"
            "lwx     $t4, $t4(%[table])        \n\t"
...
#endif // defined(__mips_dsp)  && !defined(SK_CPU_BENDIAN)
...
void SkBitmapProcState::platformProcs() {
    ...
    switch (fBitmap->config()) {
        case SkBitmap::kIndex8_Config:
            ...
            if (justDx && !doFilter) {
                fSampleProc16 = SI8_D16_nofilter_DX_mips;
                fShaderProc16 = NULL;
                if (isOpaque) {
                    fSampleProc32 = SI8_opaque_D32_nofilter_DX_mips;
                    fShaderProc32 = NULL;
                }
            }
