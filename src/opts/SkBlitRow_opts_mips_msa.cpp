...
static void S32A_D565_Opaque_mips_msa(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 8) {
        ...
        while (count >= 8) {
            ...
            dr = __builtin_msa_addv_h(sr,
                            SkMul16ShiftRound_mips_msa(dr, isa, SK_R16_BITS));
            dr = __builtin_msa_srli_h(dr, 8 - SK_R16_BITS);
            dg = __builtin_msa_addv_h(sg,
                            SkMul16ShiftRound_mips_msa(dg, isa, SK_G16_BITS));
            dg = __builtin_msa_srli_h(dg, 8 - SK_G16_BITS);
            db = __builtin_msa_addv_h(sb,
                            SkMul16ShiftRound_mips_msa(db, isa, SK_B16_BITS));
...
static void S32A_D565_Blend_mips_msa(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 8) {
        do {
            ...
            v8u16 temp8, temp9, temp10, temp11, temp12, temp13, temp14;
            v8u16 temp15, temp16, temp17;
            v4u32 v_sc_0, v_sc_1;
...
static void S32A_D565_Opaque_Dither_mips_msa(uint16_t* SK_RESTRICT dst,
    ...
    if (count >= 8) {
        ...
        dither_value[0] = dither_value[4] =
                                ((dither_scan >> (((x) & 3) << 2)) & 0xF);
        dither_value[1] = dither_value[5] =
                                ((dither_scan >> (((x + 1) & 3) << 2)) & 0xF);
        dither_value[2] = dither_value[6] =
                                ((dither_scan >> (((x + 2) & 3) << 2)) & 0xF);
        dither_value[3] = dither_value[7] =
                                ((dither_scan >> (((x + 3) & 3) << 2)) & 0xF);
